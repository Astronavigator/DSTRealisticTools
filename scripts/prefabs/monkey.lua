...
local function GetNightmareClock()end
...
local function FindTargetOfInterest(inst)
	...
	if not inst.harassplayer and not inst.components.combat.target then
		...
		local targets = FindPlayersInRange(m_pt.x, m_pt.y, m_pt.z, 25*25)
...
local function fn()
	...
	inst.components.combat.GetHitRange = function ()
		local weapon = inst.components.combat:GetWeapon()
			return (weapon == nil and inst.components.combat.hitrange)
			or (weapon.components.weapon.variedmodefn ~= nil and inst.components.combat.hitrange + weapon.components.weapon.variedmodefn(weapon).hitrange)
			or (weapon.components.weapon.hitrange ~= nil and inst.components.combat.hitrange + weapon.components.weapon.hitrange)
			or inst.components.combat.hitrange
		end
