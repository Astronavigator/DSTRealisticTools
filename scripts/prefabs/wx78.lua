...
local function onupdate(inst, dt)
    ...
    if inst.charge_time <= 0 then
        ...
        inst.components.temperature.mintemp = -6 + 40*runspeed_bonus -- от -6 до +6
    end
...
local function startovercharge(inst, duration)
    inst.charge_time = math.min(duration, TUNING.TOTAL_DAY_TIME*0.5)
...
local function onlightingstrike(inst)
    if inst.components.health ~= nil and not (inst.components.health:IsDead() or inst.components.health:IsInvincible()) then
        if inst.components.inventory:IsInsulated() then
            ...
            startovercharge(inst, inst.charge_time + TUNING.TOTAL_DAY_TIME * (.35 + .15 * math.random()))
        end
    end
end
...
local function master_postinit(inst)
    ...
    inst.components.playerlightningtarget:SetHitChance(0.1)
