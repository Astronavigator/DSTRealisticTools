...
local function onequip(inst, owner) 
    owner.AnimState:OverrideSymbol("swap_object", -- Symbol to override.
    	...
    	"arma2") -- Symbol to overwrite it with.
...
local function fn(Sim)
    ...
    inst.AnimState:PlayAnimation("arma2")
	...
	inst.save_damage = math.floor(60+math.random(0,20))
	inst.components.weapon:SetDamage(inst.save_damage)
    ...
    inst.components.finiteuses:SetMaxUses(550)
    inst.components.finiteuses:SetUses(550)
...
STRINGS.NAMES.ARMA2 = "Metallic Pipe"
STRINGS.CHARACTERS.GENERIC.DESCRIBE.ARMA2 = "Not bad."
--require "cooking"
--AddIngredientValues({"salo"},{meat=0.5,fat=1})

local mk = rawget(_G,"RegisterRussianName")
if mk then
	mk("ARMA2","Металлическая труба",3)
end

return Prefab( "common/inventory/arma2", fn, assets) 
