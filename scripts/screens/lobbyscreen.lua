...
local LobbyScreen = Class(Screen, function(self, profile, cb, no_backbutton, default_character, days_survived)
	...
	--]]
	self.players = self.fixed_root:AddChild(Text(BODYTEXTFONT, 30))
	--self.heroportait:SetScale(.9)
	self.players:SetHAlign(ANCHOR_LEFT)
	self.players:SetVAlign(ANCHOR_TOP)
	self.players:SetPosition(270, 400)
	self.players:SetRegionSize( 560,720 )
	self.players:EnableWordWrap( true )
	self.players:SetString( "xxx" )
...
function LobbyScreen:SelectPortrait(portrait)
	local character = self:GetCharacterForPortrait(portrait) --снова префаб
	...
	if character ~= nil then
		...
		self.heroportait:SetSize(560,720)
		_G.rawset(_G,"hero",self.heroportait)
